...
public:
    ...
    char* getLastError() { return this->lastError; }

    bool isFull() { return this->pointer >= stackSize; }
    ...
    T peek() { return arr[this->pointer - 1]; }

    int size() { return this->pointer; }

    bool isEmpty() { return this->pointer == 0; }

private:
    void error(const char* err) { this->lastError = const_cast<char*>(err); }
    T* arr;
    char* lastError;
    int pointer;
    const int stackSize;
